...
from apps.elephants.models import (
    Categories,
    Items,
    Fashions,
    Sizes,
...
def delivery_cost_sum(country, cart):
    dc_sum = 0
    w = cart.get_weith()
    dc = DeliveryCost.objects.filter(
        country=country, weigth_from__lte=w, weigth_to__gt=w
    )
    if dc:
        dc_sum = dc[0].cost
    else:
...
def rozetka(request):
    ...
    for offer in offers:
        ...
        if not offer.id in offers_id and not offer_name in offers_name:
            ...
            for size in Sizes.objects.select_related().filter(
                balance__item=offer, balance__amount__gt=0
            ):

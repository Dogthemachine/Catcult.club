...
class CommentForm(forms.Form):
    comment = forms.CharField(
        label=_("Comment"), max_length=1024, required=False, widget=forms.Textarea()
    )
...
class DeliveryForm(forms.Form):
    delivery = forms.TypedChoiceField(
        label=_("Delivery"), choices=lang(settings.DELIVERY), coerce=int
    )
    ttn = forms.IntegerField(label=_("TTN"), required=False)
    date = forms.DateField(
        label=_("Date of delivery"), input_formats=["%d.%m.%Y"], required=False
    )

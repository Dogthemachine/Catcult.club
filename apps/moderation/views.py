        ...
        if id and amount >= 0:
            try:
                ...
                return {
                    "success": True,
                    "message": _(
                        "Balance was saved. Old value: %(old)s, new value: %(new)s"
                    )
...
def log(request):
        ...
        date_from = list(
            map(
                ...
                datetime.datetime.strftime(
                    datetime.date.today() - datetime.timedelta(days=30), "%Y-%m-%d"
                ).split("-"),
            )
        )
    ...
    if date_to:
        ...
        date_to = list(
            map(
                int,
                datetime.datetime.strftime(datetime.date.today(), "%Y-%m-%d").split(
                    "-"
                ),
            )
        )
    ...
    return render(
        request,
        "moderation/log.html",
        {"logs": logs, "date_from": date_from, "date_to": date_to, "arrival": arrival},
    )
...
def export_balance(request):
    ...
    response[
        "Content-Disposition"
    ] = 'attachment; filename="catcult-balance-%s.csv"' % datetime.datetime.strftime(
    ...
    for balance in Balance.objects.all().order_by("item"):
        writer.writerow(
            [
                balance.item.fashions.categories,
                balance.item.fashions,
                balance.item,
                balance.size,
                balance.amount,
            ]
...
def manage_iwant(request):
    ...
    if show_archived:
        if int(status) > 0:
            iwant = IWant.objects.filter(
                added__date__gte=datetime.date(
    ...
    return render(
        request,
        "moderation/i_want.html",
        {
            "status": status,
            "iwant": iwant,
            "date_from": date_from,
            "date_to": date_to,
            "max_status": max(IWant.STATUS)[0],
...
def manage_comments(request):
    ...
    return render(
        ...
        "moderation/comments.html",
        {
            ...
            "date_from": date_from,
            "date_to": date_to,
            "max_status": max(Comments.STATUS)[0],
            "filter_status": int(filter_status),
        },
    )
...
def manage_orders(request):
    ...
    orderitems = OrderItems.objects.filter(
        order__added__gte=last.datetime, balance__amount=0
    )
    if orderitems:
...
def delete_order(request, id):
    ...
    messages.add_message(request, messages.SUCCESS, _("Order %s was deleted.") % id)
        ...
        if order_form.is_valid():
            ...
            if order.ttn > 0 and not order.sms_sent:
                if order.lang_code == "en":
                    text = (
                        "Your order #%(number)s has been sent. TTN: %(ttn)s. CatCult"
                        % {"number": order.get_number(), "ttn": order.ttn}
                    )
                elif order.lang_code == "ru":
                    text = (
                        "Ваш заказ №%(number)s был отправлен. ТТН: %(ttn)s. CatCult"
                        % {"number": order.get_number(), "ttn": order.ttn}
                    )
                elif order.lang_code == "uk":
                    text = (
                        "Ваше замовлення №%(number)s було відправлено. ТТН: %(ttn)s. CatCult"
                        % {"number": order.get_number(), "ttn": order.ttn}
                    )
                else:
                    text = ""
...
def j_order_delivery(request, id, reset=False):
    ...
    form_date = (
        order.date_of_delivery if order.date_of_delivery else datetime.date.today()
    )

    form = DeliveryForm(
        initial={"delivery": order.delivery_method, "ttn": order.ttn, "date": form_date}
    )
...
def stat_sale(request):
    ...
    for category in stat:
        ...
        for item in items:
            ...
            category_profit = (
                category_profit + item.amount * item.balance.item.fashions.cost_price
            )
    ...
    if payment == -1:
        items = (
            ...
            .exclude(order__discount_promo=0)
            .all()
        )
    else:
        items = (
    ...
    for item in items:
        total_discount_promo += int(
            item.amount * item.price * item.order.discount_promo / 100
        )
...
def stat_ending(request, rest=0):
    ...
    for balance in balances:
        ...
        balance.sl = OrderItems.objects.filter(
            balance=balance, added__gte=date_from
        ).count()

    return render(
        request,
        "moderation/stat_ending.html",
        {"balances": balances},
    )

...
class LoginForm(forms.Form):
    login = forms.CharField(
        label=_("Login"), widget=forms.TextInput(attrs={"class": "span4"})
    )
    password = forms.CharField(
        label=_("Password"), widget=forms.PasswordInput(attrs={"class": "span4"})
    )
    ...
    def clean(self):
        ...
        if login and password:
            self.user = authenticate(request, username=login, password=password)
            if self.user is None:
                # If no user was found - raise a general error
                msg = _(u"Entered password does not " + u"correspont to entered login.")
                raise forms.ValidationError(msg)
            if not self.user.is_active:
                # ... also raise it when the user is blocked.
                msg = _(
                    u"User is blocked. If you have "

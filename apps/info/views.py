    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, _("You were successfully logged in.")
            )
...
def user_logout(request):
    if not request.user.is_authenticated:
        messages.add_message(request, messages.SUCCESS, _("You are not logged in."))

    else:
        logout(request)
        messages.add_message(
            request, messages.SUCCESS, _("You were successfully logged out.")
        )

...
def showcase(request, category_id=None, fashion_id=None):
    ...
    message1 = 'Друзья, редакция ТрикотаЖ (полным составом) отправляется в отпуск. Это отличная новость (для нас), но так же это значит, '
    message1 += 'что мы не будем доступны на протяжении нескольких недель.И ещё нам пришлось положить сайт, что бы не принять заказов, '
    message1 += 'которые мы не сможем отправить (эта новость уже так себе) (но тут у нас просто нет выбора). '
    message1 += 'Так что прощаемся с вами до весны, ведите себя хорошо, смейтесь почаще и любите друг друга без стыда. '
    message1 += 'А мы к вам обязательно вернемся, покажем новые вещички из весенней серии (да, да, мы неплохо подготовились в этом году) и, конечно, что нибудь вам продадим. '
